...
%     minTrialData = min(analogData,[],1);
    ...
    if showTrials && ~isempty(goodTrials)
        ...
        subplot(8,8,i); plot(timeVals,analogDataSegment(goodTrials,:));title(['elec' num2str(i)]);
        axis('tight');
    end
end
...
if saveData
    ...
    save(fullfile(folderSegment,'badTrials.mat'),'badTrials','allBadTrials','nameElec','checkTheseElectrodes','threshold','maxLimit','minLimit','checkPeriod');
else
